// flow-typed signature: 541236601f24c67a0ee9b429160ac90c
// flow-typed version: <<STUB>>/codemirror_v5.37.0/flow_v0.71.0
...
declare module 'codemirror' {
  ...
  declare export type TextMarkerOptions = {
    atomic?: boolean,
    className?: string,
    css?: string,
    readOnly?: boolean,
  ...
  declare class Doc {
    ...
    getCursor(start?: 'anchor' | 'from' | 'to' | 'head'): Pos;
    markClean(): void;
    isClean(generation?: number): boolean;
    setValue(string): void;
    markText(from: Pos, to: Pos, options?: TextMarkerOptions): TextMarker;
  }
  ...
  declare export class MatchResult extends Array<string> {
    index?: number;
    input?: string;
  }
  ...
  declare export class Cursor {
    doc: CodeMirror;
    pos: { from: Pos, to: Pos, match?: ?MatchResult }; // undocumented
    find(reverse?: boolean): { from: Pos, to: Pos } | false; // undocumented
    findNext(): ?MatchResult;
    findPrevious(): ?MatchResult;
    to(): Pos;
    from(): Pos;
  ...
  declare export type Mode = {
    ...
    indent?: (state: State, textAfter: string) => ?string,
    innerMode?: (state: State) => {state: State, mode: string}
  }

  declare export type Events = {
  ...
  declare export type Command = (cm: CodeMirror) => void;
  declare export type Commands = {[name: string]: Command};

  declare export class Scrollbar {
    clear(): void;
  }

  declare class CodeMirror {
    static Pos(from: number, to?: number): Pos;
    static commands: Commands;
    static fromTextArea(textArea: HTMLElement, config?: CodeMirrorOptions): CodeMirror;
    static overlayMode(base: string, overlay: Mode, combine: boolean): Mode;
    static defineMode(mode: string, def: (config: CodeMirrorOptions, state: State) => Mode): void;
    static getMode(config: ?CodeMirrorOptions, backdrop?: string): string;
    options: CodeMirrorOptions;
    state: State;
    firstLine(): number;
    lastLine(): number;
    operation(() => *): *;
    scrollIntoView(options: { from: Pos, to: Pos }, unknown?: number): void;
    removeOverlay(overlay: Mode): void;
    addOverlay(overlay: Mode): void;
    showMatchesOnScrollbar(query: string | RegExp, ignoreCase: boolean): Scrollbar;
    getCursor(name?: string): Pos;
    getRange(from: Pos, to: Pos, separator?: string): string;
    addLineClass(line: number | LineHandle, where: string, className: string): LineHandle;
    getCursor(start?: 'anchor' | 'from' | 'to' | 'head'): Pos;
    findWordAt(pos: Pos): { anchor: Pos, head: Pos };
    getRange(anchor: Pos, head: Pos): string;
    replaceRange(text: string, anchor: Pos, head: Pos): void;
    ...
    findMarks(from: Pos, to: Pos): Array<TextMarker>;
    focus(): void;
    getDoc(): Doc;
    getSearchCursor(query: string | RegExp | Pos, start: ?Pos, options: ?{ caseFold?: boolean, multiline?: boolean }): Cursor;
    ...
    setSelection(anchor: Pos, head?: Pos, options?: {
      scroll?: boolean,
      origin?: string,
      bias?: -1 | 1,
    }): void;
...
declare module 'codemirror/src/measurement/Pos_measurement' {
  declare module.exports: any;
}
...
declare module 'codemirror/src/measurement/Pos_measurement.js' {
  declare module.exports: $Exports<'codemirror/src/measurement/Pos_measurement'>;
}
