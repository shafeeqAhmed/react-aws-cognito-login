...
export default class Uploader extends PureComponent<Props, State> {
  ...
  progress() {
    ...
    const progress = uploads.reduce(
      (all, one) => {
        all.total += one.total; // eslint-disable-line no-param-reassign
        all.loaded += one.loaded; // eslint-disable-line no-param-reassign
        return all;
      },
      { total: 0, loaded: 0 }
    );
  ...
  dismissUploads = () => {
    const { uploads, dismissUpload } = this.props;

    return uploads.forEach(u => {
      const fileId = get(u, "file.id");
            ...
            <div className={css.progressBar}>
              <LinearProgress
                ...
                classes={{
                  ...
                  bar2Determinate: css.progressBar2Determinate
                }}
              />
            </div>
          ...
          {open && (
            <div className={css.uploads}>
              <Table className={css.table}>
                <TableBody>
                  {uploads.map(u => {
                    const p = Uploader.normalize(u.loaded, u.total);
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <span className={css.fileName}>{u.file.name}</span>
                        </TableCell>
                        <TableCell
                          component="td"
                          scope="row"
                        ...
                        >
                          <IconButton
                            ...
                            onClick={() => this.dismissUpload(u.file.id)}
                          >
