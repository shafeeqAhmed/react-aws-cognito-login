...
export const getPositionOfCapturingGroup = (markup, parameterName, regex) => {
  ...
  if (indexDisplay === null && indexId === null) {
    ...
    throw new Error(
      `The markup \`${markup}\` must contain at least one of the placeholders \`__id__\` or \`__display__\``
    );
  }
...
export const iterateMentionsMarkup = (
  ...
  const displayPos = getPositionOfCapturingGroup(markup, "display", regex);
  const idPos = getPositionOfCapturingGroup(markup, "id", regex);
  const typePos = getPositionOfCapturingGroup(markup, "type", regex);
...
export const applyChangeToValue = (
  ...
  const oldPlainTextValue = getPlainText(
    value,
    markup,
    displayTransform,
    regex
...
export const getPlainText = (value, markup, displayTransform, regex) => {
  ...
  return value.replace(regex, function() {
    ...
    const type = typePos !== null ? arguments[typePos + 1] : null;
    if (displayTransform) display = displayTransform(id, display, type);
    return display;
  });
};
...
export const getMentions = (value, markup, displayTransform, regex) => {
  ...
  iterateMentionsMarkup(
    ...
    markup,
    () => {},
    (match, index, plainTextIndex, id, display, type, start) => {
      mentions.push({
        id,
        display,
        type,
